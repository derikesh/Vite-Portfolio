      ...
      <Flex  height={'auto'}  width={'fit-content'} margin={'auto'} alignItems={'center'} gap={{lg:20,sm:10}} className="single_exp">
        <Box display={{lg:'block',sm:'block',base:'none'}} className="exp_date" >{dataExp.date}</Box>
             <Box display={{lg:'block',sm:'block',base:'none'}} width={{lg:'100px',sm:'30px'}} height={'2px'} bg={'white'} ></Box>
                 ...
                 <Flex sx={transitionSingle} ref={(el)=>singleExpRef.current[indexNum]=el} transformOrigin={'bottom'} borderRadius={10} padding={4} bg={'#2B2B2B'} align={'top'} gap={6} height={'auto'} width={{lg:'380px',sm:'300px',base:'100%'}} >
                     ...
                     <Box transition={'0.3s ease-in'} width={'inherit'} className="exp_text_content">
                         <Heading as={'h3'} fontWeight={600} color={'#acacac'} >{dataExp.title}</Heading>
                         <Text display={{sm:click ? 'none' :'block',base:'none'}} pt={3} fontFamily={'sans-serif'} fontSize={'14px'} color={'#E6E6E6'} >( { dataExp.role } )</Text>
